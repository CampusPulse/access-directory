...
from db import db, ImageAccessPointRelation, Image, Base, Mapped
...
def upgrade():
    ...
    with op.batch_alter_table('access_point_image_relation', schema=None) as batch_op:
        ...
        batch_op.execute(db.text("""
            UPDATE access_point_image_relation 
            SET ordering = (
                SELECT ordering FROM images 
                WHERE images.id = access_point_image_relation.image_id
            )
        """))
...
def downgrade():
    ...
    with op.batch_alter_table('images', schema=None) as batch_op:
        ...
        batch_op.execute(db.text("""
            UPDATE images
            SET ordering = (
                SELECT ordering FROM access_point_image_relation
                WHERE access_point_image_relation.image_id = images.id
            )
        """))
